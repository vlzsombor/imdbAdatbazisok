...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    @Query(value = " select actors.first_name, actors.last_name, Count(movies.title) as countTitles from actors "
	    + "inner join plays on actors.actor_id = plays.actor_id "
	    + "inner join movies on movies.movie_id = plays.movie_id group by actors.first_name "
    ...
    @Query(value = 
	    ...
	    "select studios.id from studios\r\n" + 
	    "where studios.name like \"walt%\" )"
	    , nativeQuery = true)
